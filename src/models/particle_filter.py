...
class ParticleFilterMLP(nn.Module):
    ...
    def forward(self, x):
        out = self.net(x)
        raw_noise, raw_correct = self.tuple_out(out)
        noise = self.max_noise * torch.sigmoid(raw_noise)
        correction = F.softplus(raw_correct)
        return torch.concat([noise, correction], dim=-1)
