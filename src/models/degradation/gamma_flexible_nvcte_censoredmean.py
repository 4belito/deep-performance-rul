...
from src.models.degradation.censoredmean_gamma import CensoredMeanGamma
...
class GammaDegradation(DegModel):
    ...
    @classmethod
    def name(cls) -> str:
        return f"gamma_flexible_nvcte_censoredmean_onset{cls.onset_left}-{cls.onset_right}"
    ...
    @classmethod
    def build_distribution_from_params(cls, params: torch.Tensor) -> dist.Gamma:
        shape = params[..., 0]
        rate = params[..., 1]
        gamma = Gamma(shape, rate)
        return CensoredMeanGamma(gamma, cap=cls.max_life)
