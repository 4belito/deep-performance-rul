...
class RULPredictor:
    ...
    def __init__(
        ...
        self.history_component_eol: dict[str, list[tuple[float, float, float, torch.Tensor]]] = {
    ...
    def record_component_eol(self):
        ...
        for name, pf in self.pf_models.items():
            ...
            lower, pred, upper, samples = pf.mixture.uncertainty_interval(
                s0, self.conf_level, pred_stat=self.pred_stat
            )
            self.history_component_eol[name].append(
                (lower.item(), pred.item(), upper.item(), samples)
            )
    ...
    def system_eol(self) -> tuple[float, float, float]:
        ...
        for name in self.pf_models.keys():
            eol_lower, eol_pred, eol_upper, samples = self.history_component_eol[name][-1]
            lowers.append(eol_lower)
            preds.append(eol_pred)
            uppers.append(eol_upper)
            perform_eol_samples.append(samples)
        perform_eol_samples = torch.stack(perform_eol_samples, dim=0)
        eol_samples = self.eol_aggregation(perform_eol_samples)
        eol_lower = torch.quantile(eol_samples, self.alpha / 2, dim=0)
        eol_pred = torch.mean(eol_samples, dim=0)
        eol_upper = torch.quantile(eol_samples, 1 - self.alpha / 2, dim=0)
        # Physical bounds
    ...
    @torch.no_grad()
    def eol_aggregation(self, samples: torch.Tensor) -> torch.Tensor:
        return torch.min(samples, dim=0).values
